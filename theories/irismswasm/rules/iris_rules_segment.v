...
From MSWasm.iris.rules Require Export iris_rules_resources iris_rules_structural.
From Coq Require Import BinNat.
From MSWasm Require Import opsem operations datatypes properties handle list_extra.
...
Qed.
 ...
 Proof.
   unfold isAlloc, segment_list.find. intro H; rewrite H => //. 
 Qed.

 Lemma in_allocated_None_implies_isNotAlloc i a:
   a.(allocated) !! i = Some None -> isNotAlloc i a.
 Proof.
   unfold isNotAlloc, segment_list.find. intro H; rewrite H => //.
 Qed. 
...
Lemma wp_segload_failure1 (Φ: iris.val -> iProp Σ) (s:stuckness) (E:coPset) (t: value_type) h (f: frame):
  handle.valid h = false \/ (offset h + N.of_nat (t_length t) > bound h)%N \/ t = T_handle /\ (N.modulo (handle_addr h) handle_size <> N.of_nat 0)%N ->
  (▷ Φ trapV ∗ ↪[frame] f ⊢
...
Definition plus_one h :=
  {| base := base h ; offset := offset h + 1 ; bound := bound h ; handle.valid := handle.valid h;
                                                                  id := id h |}.
...
Proof.
  ...
  destruct (seg_lookup (handle_addr h + N.of_nat 0) (seg_data m)) ; inversion Hm.
  rewrite list_extra.cons_app in H0. apply (those_app_inv [Some p]) in H0.
  destruct H0 as (tl1 & tl2 & Htl1 & Htl2 & Htl).
...
Proof. 
  ...
  rewrite Hbs. 
  unfold drop.
  iFrame. rewrite Nat2N.inj_add. rewrite N.add_assoc. done. 
  { intros y Hy Habs.
...
Lemma wp_segfree_failure1 h f0 Φ s E :
   h.(offset) <> N.zero \/ h.(handle.valid) = false ->
    ▷ (Φ trapV) ∗ ↪[frame] f0
     ⊢ (WP [AI_const (VAL_handle h) ;
            AI_basic BI_segfree ] @ s; E {{ w, Φ w ∗ ↪[frame] f0 }}).
Proof.
...
Proof.
  ...
  { apply N.eqb_eq in Hi as ->.
    ...
    replace ( ((#00%byte, Numeric) :: repeat (#00%byte, Numeric) size)%SEQ)
      with(repeat (#00%byte, Numeric) (S size)); last done.
    apply repeat_lookup.
    rewrite take_length_le. 
    lia. done. simpl. rewrite repeat_length. rewrite take_length_le. lia.
  ...
  { symmetry. apply map_filter_lookup_Some.
    ...
    { apply N.ltb_lt in Hi'.
      ...
      { apply N.ltb_lt in Hi''.
        ...
        rewrite app_comm_cons lookup_app_l in H0; last by simpl; rewrite repeat_length take_length_le; lia.
        rewrite take_length_le; last lia.
        rewrite take_length_le in H0; last lia.
        assert (N.to_nat i - N.to_nat addr < S size); first lia.
      ...
      rewrite app_comm_cons lookup_app_r in H0; last by simpl; rewrite repeat_length take_length_le; lia.
      rewrite repeat_length. simpl in H0. rewrite repeat_length in H0.
      rewrite lookup_drop. rewrite lookup_drop in H0.
      rewrite take_length_le in H0; last lia.
      replace (N.to_nat (addr + N.of_nat size) + (N.to_nat i - N.to_nat addr - size))
        with (N.to_nat (addr + N.of_nat (S size)) +
                (N.to_nat i - N.to_nat addr - S size));
        [done | lia]. } 
    ...
    2:{ left.
        ...
        repeat rewrite app_length. rewrite app_length in gmi. simpl in gmi. rewrite app_length in gmi.
