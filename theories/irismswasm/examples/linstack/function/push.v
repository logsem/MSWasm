...
From MSWasm.iris.examples.linstack Require Export stack_common is_full.
...
*)
  ...
  Section specs.
    ...
    Proof.
      ...
      - iIntros (w) "[-> Hf]".
        ...
        iSplitR "HΦ".
      - iApply wp_wand_r. iSplitL. fold_const. iApply wp_store ; last first.
        iFrame.
        iSplitR "Ha".
        iNext.
        subst x.
  ...
  Section valid.
    ...
    - iIntros (f0) "(Hf & -> & Hown & Hall)".
      ...
      iIntros (?) "[-> $]". iSplitR; last by iExists _. iLeft. iLeft. auto.
    - iIntros "Hf". iFrame.
    ...
    - subst f'. iIntros (w f0) "([-> %Hdiv] & Hf & -> & Hown & Hall) /=".
      ...
      + apply div_mod_i32 in Hdiv as Hdiv'.
        ...
        * iApply (spec_push_op with "[$Hstack $Hf]").
          ...
          iLeft. iRight. iExists []. simpl. done.
        * iDestruct (stack_pure with "Hstack") as "(_ & _ & %Hstkbound & Hstack)".
          ...
          by iIntros "!>" (?) "%". iIntros (v') "[-> Hf]".
          deconstruct_ctx.
          iApply (wp_wand _ _ _ (λ v, ⌜v = trapV⌝ ∗ _)%I with "[Hf]").
          iApply (wp_label_trap with "[$]");eauto.
          iIntros (v0) "[-> Hf]".
          iExists _. iFrame. iIntros "Hf". iApply (wp_wand with "[Hf]").
