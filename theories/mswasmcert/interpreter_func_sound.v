(** soundness of the MSWasm interpreter **)
(* adapted from (C) J. Pichon, M. Bodin, Rao Xiaojia *)
...
From MSWasm Require Import operations properties opsem properties stdpp_aux type_preservation.
From MSWasm Require Import interpreter_func.
  ...
  - explode_and_simplify; try by intro H ; inversion H.
    ...
    + destruct (run_one_step (run_one_step_fuel e) d (s, f, rev lconst) e)
      ...
      * move: E. rewrite /interpreter_func.run_step_fuel HSplitVals.
        ...
        move=> E. have: exists v, n = PeanoNat.Nat.max (run_one_step_fuel e) v.
        {
...
Proof.
  ...
  destruct fuel.
  { destruct fuel'; last lias.
    move => d s f es s' f' es' x H.
    inversion H. }
  generalize dependent fuel'. 
  induction fuel; intros fuel' Hfuel.
  {
     move=> d s f es s' f' es' HWF /=.
  destruct fuel' as [|fuel'] => //=.
  unfold run_step_with_fuel. unfold interpreter_func.run_step_with_fuel.
  destruct (split_vals_e es) as [lconst les] eqn:HSplitVals.
  ...
  destruct fuel' as [|fuel'] => //. destruct e as [b| | |cl|n es1 es2|n f0 ess|].
        ...
        + destruct (rev lconst) eqn:Hlconst; try by destruct v => //.
          ...
          * do 2 (move/andP in Hb; destruct Hb as [Hb ?]).
            ...
            -- intro Htuple. split; last by destruct v; inversion Htuple; subst.
               ...
               apply BinNat.N.leb_le in H0.
               apply plus_binnat_leq => //.
               unfold isAlloc. unfold isAllocb in H. by destruct (find _ _).
               exact Hsegload.
               eapply (rm_label (k := 0) (lh := LH_base (v_to_e_list _) _)) ; last first.
               unfold lfilled, lfill.
               rewrite v_to_e_is_const_list.
        ...
        + destruct v => //.
          ...
          * do 3 (move/andP in Hb; destruct Hb as [Hb ?]).
            ...
            -- intro Htuple; inversion Htuple; subst.
               ...
               unfold isAlloc. unfold isAllocb in H0. by destruct (find _ _).
               apply Neqb_ok in H. exact H. simpl in Htuple.
               assert (forall a b, s_segs (upd_s_seg a b) = b).
               intros a1 b; by destruct a1.
               unfold wellFormedState. rewrite H2.
               unfold wellFormedState in HWF.
    ...
    |}) as Halloc.
      ...
      - (** [AI_basic BI_segfree] **)
        simpl; explode_and_simplify; pattern_match; auto_frame.
        destruct p as [[??]?].
        destruct (BinNat.N.eqb n (base h) && BinNat.N.eqb _ (bound h) ) eqn:Hn0; try by inversion H1.
        inversion H1; subst.
      ...
      - (** [AI_basic (Cvtop v c v0 o)] **)
        ...
        apply andb_prop in H0 as [??]. destruct o => //.
    ...
    { (** [Label] **)
      ...
      + destruct run_step_with_fuel as [[s'' f''] r] eqn: EH.
        ...
        * (** [RS_normal] **)
          ...
          apply IHfuel in EH as [??]; try by lias.
          split; last done.
          destruct H0 as [me EH]. exists me. eapply rm_label.
          - by eauto.
          - apply/lfilledP.
    ...
    { (** [Local] **)
      ...
      - split; last by subst. exists ME_empty.
        ...
        do 2 econstructor.
        constructor.
        done. apply NPeano.Nat.eqb_eq in if_expr2. done.
      - destruct run_step_with_fuel as [[s'' f''] r] eqn: EH.
        destruct r as [| |vs'''|es''| ] => //.
        * (** [RS_return] **)
          explode_and_simplify.
        ...
        * (** [RS_normal] **)
          inversion H1 ; subst. auto_frame.
          apply IHfuel in EH as [??]; try by lias.
          split.
          destruct H0 as [me EH]. exists me.
          eapply (rm_label (k := 0) (lh := LH_base (v_to_e_list _) _)) ; last first.
          unfold lfilled, lfill.
          rewrite v_to_e_is_const_list.
          instantiate ( 2 := [:: _]) => //=.
