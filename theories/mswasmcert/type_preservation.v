...
From MSWasm Require Export common.
From mathcomp Require Import ssreflect ssrfun ssrnat ssrbool eqtype seq.
From Coq Require Import Program.Equality NArith ZArith_base ssreflect.
From MSWasm Require Export operations datatypes_properties typing opsem properties.
Require Import Lia.
From MSWasm Require Import inj_div. 
...
Ltac fold_const_in v H := fold (AI_const (VAL_numeric v)) in H. 
...
Lemma BI_const_typing: forall C econst t1s t2s,
    ...
    t2s = t1s ++ [::typeof_numerical econst].
Proof.
...
Lemma AI_handle_typing: forall s C econst t1s t2s,
    e_typing s C [::AI_handle econst] (Tf t1s t2s) ->
    t2s = t1s ++ [::T_handle].
Proof.
...
Proof.
  move => s C econst t1s t2s HType.
  gen_ind_subst HType => //; try by destruct econst => //. 
  - destruct bes => //. destruct bes => //.
    inversion H3. destruct econst => //. inversion H1; subst.
    apply BI_const_typing in H. done. 
  - apply extract_list1 in H2; inversion H2; subst.
    apply empty_typing in HType1; subst.
    by eapply IHHType2.
  - rewrite - catA. f_equal.
...
Lemma BI_const2_typing: forall C econst1 econst2 t1s t2s,
    ...
    t2s = t1s ++ [::typeof_numerical econst1; typeof_numerical econst2].
Proof.
...
Proof.
  ...
  - destruct bes => //. destruct bes => //.  destruct bes => //.
    inversion H3. destruct econst1, econst2 => //.
    inversion H1; inversion H2; subst.
    apply BI_const2_typing in H => //. 
  - apply extract_list2 in H2; inversion H2; subst.
    apply AI_const_typing in HType1; subst.
    apply AI_const_typing in HType2; subst.
    by rewrite -catA.
  - rewrite - catA. f_equal.
    by eapply IHHType.
Qed.
...
Lemma BI_const3_typing: forall C econst1 econst2 econst3 t1s t2s,
    ...
    t2s = t1s ++ [::typeof_numerical econst1; typeof_numerical  econst2; typeof_numerical econst3].
Proof.
...
Proof.
  ...
  gen_ind_subst HType => //.
  - repeat (destruct bes => //). destruct econst1, econst2, econst3 => //.
    inversion H3; subst. apply BI_const3_typing in H => //. 
  - apply extract_list3 in H2; inversion H2; subst.
    apply AI_const2_typing in HType1; subst.
    apply AI_const_typing in HType2; subst.
    by rewrite -catA.
  - rewrite - catA. f_equal.
    by eapply IHHType.
...
Lemma Const_list_typing_empty: forall x C vs,
    e_typing x C (v_to_e_list vs) (Tf [::] (vs_to_vts vs)).
Proof.
  move => x C vs.
...
Lemma Handleadd_typing: forall C t1s t2s,
    be_typing C [::BI_handleadd] (Tf t1s t2s) ->
    exists ts, t1s = ts ++ [::T_i32 ; T_handle] /\ t2s = ts ++ [::T_handle].
Proof.
...
Lemma Isdummy_typing: forall C t1s t2s,
    be_typing C [::BI_isdummy] (Tf t1s t2s) ->
    exists ts, t1s = ts ++ [::T_handle] /\ t2s = ts ++ [::T_i32].
Proof.
  move => C t1s t2s HType.
  gen_ind_subst HType => //=.
...
Lemma Slice_typing: forall C t1s t2s,
    ...
    exists ts, t1s = ts ++ [::T_handle; T_i32 ; T_i32] /\ t2s = ts ++ [::T_handle].
Proof.
  move => C t1s t2s HType.
  gen_ind_subst HType => //=.
  - by exists [::].
...
Lemma Const_list_typing: forall sr C vs t1s t2s,
    e_typing sr C ((v_to_e_list vs)) (Tf t1s t2s) ->
    t2s = t1s ++ (map typeof vs).
Proof.
  move => sr C vs.
  induction vs => //=; move => t1s t2s HType.
  - remember [::] as l. remember (Tf t1s t2s) as t.
    generalize dependent t2s; generalize dependent t1s.
    induction HType; inversion Heql; subst; intros.
    + destruct bes => //. subst. apply empty_btyping in H. subst. by rewrite cats0.
    + destruct es => //.
    + inversion Heqt; subst. rewrite (IHHType _ t1s t2s) => //.
      by rewrite catA.
...
Ltac invert_be_typing:=
  ...
  | H: be_typing _ [::BI_unop _ _] _ |- _ =>
    ...
    | H: be_typing _ [::BI_handleadd] _ |- _ =>
        let ts := fresh "ts" in
        let H1 := fresh "H1" in
        let H2 := fresh "H2" in
    ...
    | H: be_typing _ [::BI_isdummy] _ |- _ =>
        let ts := fresh "ts" in
        let H1 := fresh "H1" in
        let H2 := fresh "H2" in
        apply Isdummy_typing in H; destruct H as [ts [H1 H2]]; subst 
    | H: be_typing  _ [::BI_slice] _ |- _ =>
        let ts := fresh "ts" in
        let H1 := fresh "H1" in
        let H2 := fresh "H2" in
        apply Slice_typing in H; destruct H as [ts [H1 H2]]; subst
...
Proof.
  move => op v1 v2 v.
  by elim: op; elim: v1; elim: v2 => //=; move => c1 c2 op H; destruct c1, c2, op ; remove_bools_options.
Qed.


Lemma t_Unop_preserve: forall s C v t op be tf,
    e_typing s C [:: AI_const v; AI_basic (BI_unop t op)] tf ->
    reduce_simple ([::AI_const v; AI_basic (BI_unop t op)]) ([::be]) ->
    e_typing s C [::be] tf.
Proof.
  ...
  apply et_to_bet in Hop; last by split; eexists.
  apply Unop_typing in Hop as (-> & tsb & ->).
  apply AI_const_typing in Hv.
  apply concat_cancel_last in Hv as [-> ->].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 3 (destruct vs => //).
  - assert (v0 = v) as ->; first by destruct v,v0; inversion H0.
    replace (typeof v) with (typeof (app_unop op v)); first apply ety_const.
    destruct v,op; try destruct n; try destruct u => //.
  - unfold lfilled, lfill in H2. destruct lh => //. destruct (const_list l) => //.
    apply b2p in H2. destruct l; first by destruct v. destruct l => //. destruct l => //. 
Qed. 
...
Lemma t_Binop_preserve_success: forall s C v1 v2 t op be tf,
    e_typing s C [:: AI_const v1; AI_const v2; AI_basic (BI_binop t op)] tf ->
    reduce_simple ([::AI_const v1; AI_const v2; AI_basic (BI_binop t op)]) ([::be]) ->
    e_typing s C [::be] tf.
Proof.
  move => s C v1 v2 t op be tf HType HReduce.
  destruct tf as [ts1 ts2].
  rewrite (separate2 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Binop_typing in Hop as (-> & tsb & ->).
  apply AI_const2_typing in Hv.
  rewrite (separate1 (typeof _)) in Hv.
  rewrite - cat_app in Hv. 
  rewrite catA in Hv. 
  apply concat_cancel_last in Hv as [Hv ->].
  apply concat_cancel_last in Hv as [-> ->].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 4 (destruct vs => //).
  - apply app_binop_type_preserve in H0.
    const_equal v0 v1 H. const_equal v3 v2 H1. 
    rewrite -H0. 
    by apply ety_const.
  - apply ety_trap.
  - unfold lfilled, lfill in H2. destruct lh => //. destruct (const_list _) => //.
    apply b2p in H2. destruct l; first by destruct v1. destruct l; first by destruct v2.
    destruct l => //. destruct l => //. 
Qed. 

Lemma t_Testop_i32_preserve: forall C c testop tf,
    be_typing C [::BI_const (NVAL_int32 c); BI_testop T_i32 testop] tf ->
    be_typing C [::BI_const (NVAL_int32 (wasm_bool (app_testop_i testop c)))] tf.
Proof.
...
Lemma t_Testop_i64_preserve: forall C c testop tf,
    be_typing C [::BI_const (NVAL_int64 c); BI_testop T_i64 testop] tf ->
    be_typing C [::BI_const (NVAL_int32 (wasm_bool (app_testop_i testop c)))] tf.
Proof.
  move => C c testop tf HType.
  gen_ind_subst HType.
...
Lemma t_Relop_preserve: forall s C v1 v2 t be op tf,
    e_typing s C [::AI_const v1; AI_const v2; AI_basic (BI_relop t op)] tf ->
    reduce_simple [:: AI_const v1; AI_const v2; AI_basic (BI_relop t op)] [::be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C v1 v2 t be op tf HType HReduce.
  destruct tf as [ts1 ts2].
  rewrite separate2 in HType. 
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Relop_typing in Hop as (tsb & -> & ->). 
  apply AI_const2_typing in Hv.
  rewrite (separate1 (typeof _)) in Hv.
  rewrite - cat_app in Hv. 
  rewrite catA in Hv.
  rewrite (separate1 t) in Hv.
  rewrite - cat_app catA in Hv. 
  apply concat_cancel_last in Hv as [Hv ->].
  apply concat_cancel_last in Hv as [-> Htof].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 4 (destruct vs => //).
  - fold_const. apply ety_const' => //.
  - apply ety_trap.
Qed.
...
Lemma be_typing_const_deserialise: forall s C v t,
    e_typing s C [:: AI_const (wasm_deserialise (bits v) t)] (Tf [::] [:: t]).
Proof.
  intros. apply ety_const' => //. apply typeof_deserialise.
Qed. 

Lemma t_Convert_preserve: forall s C v t1 t2 sx be tf,
    e_typing s C [::AI_const v; AI_basic (BI_cvtop t2 CVO_convert t1 sx)] tf ->
    reduce_simple [::AI_const v; AI_basic (BI_cvtop t2 CVO_convert t1 sx)] [::be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C v t1 t2 sx be tf HType HReduce.
  destruct tf as [ts1 ts2].
  rewrite (separate1 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Cvtop_typing in Hop as (tsb & -> & ->).
  apply AI_const_typing in Hv.
  apply concat_cancel_last in Hv as [-> ->].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 3 (destruct vs => //).
  - const_equal v0 v H.
    apply ety_const'.
    destruct t2; try by simpl in H5;
      match type of H5 with
        option_map _ ?e = _ => destruct e eqn:HDestuct => //=
      end; inversion H5.
    inversion H5.
  - const_equal v0 v H.
    apply ety_trap.
...
Lemma t_Reinterpret_preserve: forall s C v t1 t2 be tf,
    e_typing s C [::AI_const v; AI_basic (BI_cvtop t2 CVO_reinterpret t1 None)] tf ->
    reduce_simple [::AI_const v; AI_basic (BI_cvtop t2 CVO_reinterpret t1 None)] [::be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C v t1 t2 be tf HType HReduce.
   destruct tf as [ts1 ts2].
  rewrite (separate1 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
...
Lemma t_Select_preserve: forall s C v1 v2 n tf be,
    e_typing s C [::AI_const v1; AI_const v2; AI_basic (BI_const (NVAL_int32 n)); AI_basic BI_select] tf ->
    reduce_simple [::AI_const v1; AI_const v2; AI_basic (BI_const (NVAL_int32 n)); AI_basic BI_select] [::be]->
    e_typing s C [::be] tf.
Proof.
  ...
  fold (AI_const (VAL_numeric (NVAL_int32 n))) in Hv. apply AI_const3_typing in Hv.
  rewrite (separate2 (typeof _)) in Hv.
  rewrite - cat_app in Hv. 
  rewrite catA in Hv.
  ...
  - const_equal v0 v1 H. const_equal v3 v2 H1.  apply ety_const. 
  - const_equal v0 v1 H. const_equal v3 v2 H1. rewrite Hto. apply ety_const. 
  - unfold lfilled, lfill in H2. destruct lh => //. destruct (const_list _) => //.
    apply b2p in H2. destruct l; first by destruct v1. destruct l; first by destruct v2.
...
Lemma t_If_be_preserve: forall C c tf0 es1 es2 tf be,
  be_typing C ([::BI_const (NVAL_int32 c); BI_if tf0 es1 es2]) tf ->
  reduce_simple (to_e_list [::BI_const (NVAL_int32 c); BI_if tf0 es1 es2]) [::AI_basic be] ->
  be_typing C [::be] tf.
Proof.
...
Lemma t_Br_if_true_preserve: forall C c i tf be,
    be_typing C ([::BI_const (NVAL_int32 c); BI_br_if i]) tf ->
    reduce_simple (to_e_list [::BI_const (NVAL_int32 c); BI_br_if i]) [::AI_basic be] ->
    be_typing C [::be] tf.
Proof.
...
Lemma t_Br_if_false_preserve: forall C c i tf,
    be_typing C ([::BI_const (NVAL_int32 c); BI_br_if i]) tf ->
    reduce_simple (to_e_list [::BI_const (NVAL_int32 c); BI_br_if i]) [::] ->
    be_typing C [::] tf.
Proof.
...
Lemma t_Br_table_preserve: forall C c ids i0 tf be,
    be_typing C ([::BI_const (NVAL_int32 c); BI_br_table ids i0]) tf ->
    reduce_simple (to_e_list [::BI_const (NVAL_int32 c); BI_br_table ids i0]) [::AI_basic be] ->
    be_typing C [::be] tf.
Proof.
...
Proof.
  ...
  + instantiate (1 := ([::typeof v])). apply ety_const. 
  + instantiate (1 := ([::typeof v] ++ [::typeof v])).
    rewrite <- (cats0 [:: typeof v]) at 1. apply ety_weakening. apply ety_const. 
  + apply et_weakening_empty_2. apply ety_a'; first by split; eexists.
    by apply bet_set_local.
Qed. 

 Lemma t_Handleadd_preserve: forall s C c h be tf,
    e_typing s C ([::AI_const (VAL_int32 c); AI_const (VAL_handle h); AI_basic BI_handleadd]) tf ->
    reduce_simple ([::AI_const (VAL_int32 c); AI_const (VAL_handle h); AI_basic BI_handleadd]) [:: be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C c h be tf HType HReduce.
   destruct tf as [ts1 ts2].
  rewrite (separate2 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Handleadd_typing in Hop as (tsb & -> & ->).
  apply AI_const2_typing in Hv.
  rewrite (separate1 (typeof _)) in Hv.
  rewrite - cat_app in Hv. 
  do 2 rewrite catA in Hv. 
  apply concat_cancel_last in Hv as [Hv _].
  apply concat_cancel_last in Hv as [-> _].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 4 (destruct vs => //).
  - fold_const. apply ety_const => //.
  - apply ety_trap. 
  - unfold lfilled, lfill in H2. destruct lh => //. destruct (const_list _) => //.
    apply b2p in H2. repeat (destruct l => //). 
Qed. 


Lemma t_Slice_preserve: forall s C c1 c2 h be tf,
    e_typing s C ([:: AI_const (VAL_handle h); AI_const (VAL_int32 c1) ; AI_const (VAL_int32 c2) ; AI_basic BI_slice]) tf ->
    reduce_simple ([::AI_const (VAL_handle h); AI_const (VAL_int32 c1) ; AI_const (VAL_int32 c2); AI_basic BI_slice]) [:: be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C c1 c2 h be tf HType HReduce.
  destruct tf as [ts1 ts2].
  rewrite (separate3 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Slice_typing in Hop as (tsb & -> & ->).
  apply AI_const3_typing in Hv.
  rewrite (separate2 (typeof _)) in Hv.
  rewrite - cat_app in Hv. 
  do 2 rewrite catA in Hv.
  apply concat_cancel_last in Hv as [Hv _].
  rewrite (separate1 (typeof _)) in Hv.
  rewrite - cat_app catA catA in Hv. 
  apply concat_cancel_last in Hv as [Hv _].
  apply concat_cancel_last in Hv as [-> _].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 5 (destruct vs => //).
...
Lemma t_Getoffset_preserve: forall s C h be tf,
    e_typing s C ([:: AI_const (VAL_handle h); AI_basic BI_getoffset]) tf ->
    reduce_simple ([::AI_const (VAL_handle h); AI_basic BI_getoffset]) [:: be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C h be tf HType HReduce.
  destruct tf as [ts1 ts2].
  rewrite (separate1 (AI_const _)) in HType.
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Getoffset_typing in Hop as (tsb & -> & ->).
  apply AI_const_typing in Hv.
  apply concat_cancel_last in Hv as [-> _].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 3 (destruct vs => //).
...
Lemma t_Isdummy_preserve: forall s C h be tf,
    e_typing s C ([:: AI_const (VAL_handle h); AI_basic BI_isdummy]) tf ->
    reduce_simple ([::AI_const (VAL_handle h); AI_basic BI_isdummy]) [:: be] ->
    e_typing s C [::be] tf.
Proof.
  move => s C h be tf HType HReduce.
  destruct tf as [ts1 ts2].
  ...
  eapply e_composition_typing in HType as (ts & ts1' & ts2' & tres & -> & -> & Hv & Hop).
  apply et_to_bet in Hop; last by split; eexists.
  apply Isdummy_typing in Hop as (tsb & -> & ->).
  apply AI_const_typing in Hv.
  apply concat_cancel_last in Hv as [-> _].
  rewrite catA. apply et_weakening_empty_1.
  inversion HReduce; subst; try by do 3 (destruct vs => //).
...
Lemma const_list_rev l : const_list (rev l) -> const_list l.
Proof.
  unfold rev. remember [::] as l'. intros H. 
  assert (const_list l' /\ const_list l) as Hres ; last by destruct Hres.
  clear Heql'. 
  generalize dependent l'. induction l => //=.
  intros l' H. apply IHl in H as [??]. simpl in H.
  apply Bool.andb_true_iff in H as [-> ->]. rewrite H0. done.
Qed. 
...
Proof.
  ...
  - subst. 
    ...
    + simpl in H0. move/andP in H0. destruct H0.
      destruct a => //=.
      destruct b => //=.
      fold (AI_const (VAL_numeric n)) in H4. 
      apply AI_const_typing in H4. subst.
      rewrite - (cats0 t3s). rewrite - catA.
      apply ety_weakening.
      apply ety_a' => //. by split; eexists. by apply bet_const.
      apply AI_handle_typing in H4. subst.
      rewrite - (cats0 t3s) - catA. apply ety_weakening.
      econstructor. 
Qed. 
...
Proof.
  ...
  - (* Binop *)
    eapply t_Binop_preserve_success. exact HType. exact HReduce.
  - (* Testop i32 *)
    apply ety_a' => //=. split; eexists => //. eapply t_Testop_i32_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
  - (* Testop i64 *)
    apply ety_a' => //=. split; eexists => //. eapply t_Testop_i64_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
  - (* Relop *)
    eapply t_Relop_preserve. exact HType. exact HReduce.
  - (* Cvtop *)
    eapply t_Convert_preserve. exact HType. exact HReduce.
  - (* Reinterpret *)
    eapply t_Reinterpret_preserve. exact HType. exact HReduce.
  - (* Nop *)
    apply et_to_bet in HType; last by split; eexists. simpl in HType.
    destruct tf. eapply Nop_typing in HType as ->.
    rewrite - (cats0 r0). apply ety_weakening. apply ety_a' => //. constructor.
  - (* Drop *)
  ...
  - (* Block *)
    ...
    apply et_to_bet in H5; last by split; eexists. apply Block_typing in H5.
    ...
    + move/andP in H. destruct H.
      ...
      -- eapply et_composition'; first by apply Const_list_typing_empty.
            ...
            destruct Heqes'. apply ety_a' => //. by subst.
      -- done. 
  - (* Loop *)
    destruct tf.
    apply e_composition_typing in HType.
    ...
    apply const_es_exists in H. destruct H. subst.
    apply Const_list_typing in H4. subst.
    apply et_to_bet in H5; last by split; eexists. 
  ...
  - (* If *)
     apply ety_a' => //=. split; eexists => //. eapply t_If_be_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
    exact HReduce.
  - (* If *)
     apply ety_a' => //=. split; eexists => //. eapply t_If_be_preserve.
  ...
  - (* Label_const *)
    ...
    + (* ety_composition *)
      apply extract_list1 in Hremes. destruct Hremes. subst.
      apply et_to_bet in HType1 => //.
      simpl in HType1. apply (ety_a s0) in HType1. apply empty_typing in HType1. subst.
      by eapply IHHType2 => //.
    + (* ety_weakening *)
      apply ety_weakening.
      by eapply IHHType => //.
  ...
  - (* Label_lfilled_Break *)
    ...
    + (* ety_label *)
      ...
      -- by apply HType1.
  - (* Br_if *)
    apply ety_a' => //=. eapply t_Br_if_false_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
    exact HReduce.
  - (* Br_if *)
    apply ety_a' => //=. split; eexists => //. eapply t_Br_if_true_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
    exact HReduce.
  - (* Br_table *)
    apply ety_a' => //=. split; eexists => //. eapply t_Br_table_preserve.
    apply et_to_bet in HType. exact HType. repeat (try split; eexists).
    exact HReduce.
      - (* Br_table *)
    apply ety_a' => //=. split; eexists => //. eapply t_Br_table_preserve.
  ...
  - (* Tee_local *)
    destruct v => //.
    destruct b => //.
    all: fold_const; eapply t_Tee_local_preserve. all: exact HType.
  - (* Handleadd *)
    eapply t_Handleadd_preserve. exact HType. done.
  - (* Slice *)
    eapply t_Slice_preserve. exact HType. done.
  - (* GetOffset *)
    eapply t_Getoffset_preserve. exact HType. done.
  - (* Isdummy *)
    eapply t_Isdummy_preserve. exact HType. done.
  - (* Isdummy *)
    eapply t_Isdummy_preserve. exact HType. done.
Qed. 
...
Proof.
  ...
  destruct g_mut => //=; remove_bools_options;
...
Lemma all2_seg_extension_same: forall t,
    all2 seg_extension t t.
Proof.
  move => t.
...
Lemma all2_all_extension_same: forall t,
    all2 all_extension t t.
Proof.
  move => t.
  apply reflexive_all2_same. unfold reflexive. move => x.
  done.
...
Proof.
  ...
  - move => s C v s' HST1 HST2 Hext.  fold_const; apply ety_const' => //. 
...
Proof.
  ...
  - assert ((n.+1 < length (g0 :: sglobs))%coq_nat); first by rewrite -List.nth_error_Some; rewrite HN.
    ...
    simpl.
    apply/andP. split.
    + unfold glob_extension. destruct (g_mut g0) => //=. destruct (g_val g0).
      destruct n0 => //. done. 
...
Lemma seg_extension_update_nth: forall ssegs n m m',
  List.nth_error ssegs n = Some m ->
  seg_extension m m' ->
  all2 seg_extension ssegs (update_list_at ssegs n m').
Proof.
  move => ssegs n.
  generalize dependent ssegs.
  induction n; move => ssegs m m' HN Hext => //=; destruct ssegs => //.
  - simpl in HN. inversion HN. subst.
    apply/andP. split; first apply Hext.
   by apply all2_seg_extension_same.
  - assert ((n.+1 < length (s :: ssegs))%coq_nat); first by rewrite -List.nth_error_Some; rewrite HN.
    rewrite -update_list_at_is_set_nth; last by lias.
    simpl.
    apply/andP. split.
...
Lemma all_extension_update_nth: forall salls n m m',
  List.nth_error salls n = Some m ->
  all_extension m m' ->
  all2 all_extension salls (update_list_at salls n m').
Proof.
  move => ssegs n.
  generalize dependent ssegs.
  induction n; move => ssegs m m' HN Hext => //=; destruct ssegs => //.
...
Definition proj2_some {A:Type} (acc: nat * option A) : Prop :=
  let (n, om) := acc in
  exists m, om = Some m.
...
Definition seg_type_proj2_preserve (acc1 acc2: nat * option segment_list.segment_list) : Prop :=
  let (n1, om1) := acc1 in
  let (n2, om2) := acc2 in
  (exists m1 m2,
      om1 = Some m1 /\
        om2 = Some m2 /\
        segment_list.seg_length m1 = segment_list.seg_length m2).
...
Lemma seg_type_proj2_preserve_trans: forall a1 a2 a3,
    proj2_some a1 ->
    proj2_some a2 ->
    proj2_some a3 ->
    seg_type_proj2_preserve a1 a2 ->
    seg_type_proj2_preserve a2 a3 ->
    seg_type_proj2_preserve a1 a3.
Proof.
  unfold mem_type_proj2_preserve, proj2_some.
  move => a1 a2 a3 HP1 HP2 HP3 HR12 HR23.
  destruct a1, a2, a3 => /=.
...
Lemma write_bytes_preserve_type: forall m pos str m',
  ...
  (mem_length m = mem_length m') /\ (mem_size m = mem_size m') /\ (mem_max_opt m = mem_max_opt m').
Proof.
...
Lemma is_Some_is_not_None {A} x (y : A) :
  x = Some y -> x <> None.
Proof. intro; intro. by subst. Qed.

Lemma write_segbytes_preserve_type: forall m pos str m',
  write_segbytes m pos str = Some m' ->
  (operations.seg_length m = operations.seg_length m') /\ (seg_size m = seg_size m') /\ (seg_max_opt m = seg_max_opt m').
Proof.
  ...
  assert (HPreserve: seg_type_proj2_preserve (0, Some (seg_data m)) fold_res).
  symmetry in Heqfold_res.
  destruct fold_res; subst.
  eapply list_fold_left_restricted_trans with (P := proj2_some); unfold proj2_some; eauto.
  - move => a e a' HP1 HP2 HF.
    unfold seg_type_proj2_preserve.
    destruct a as [n1 m1]. destruct a' as [n2 m2].
    destruct HP1 as [m3 HP1]; destruct HP2 as [m4 HP2].
    subst.
    injection HF. move => H1 H2. subst. clear HF.
    unfold seg_update in H1.
    destruct (pos + N.of_nat n1 <? N.of_nat (length (segl_data m3)))%N eqn:HMemSize => //=.
    apply N.ltb_lt in HMemSize.
    assert (N.to_nat (pos + N.of_nat n1) < length (segl_data m3))%coq_nat; first lias.
    edestruct List.nth_error_Some as [_ H'].
    specialize (H' H).
    destruct (List.nth_error _ _) eqn:Hnth => //.
    injection H1. move => H2. clear H1. subst.
    unfold seg_length => /=.
    repeat eexists.
  ...
  - move => a HP. destruct a as [n1 m1].
    ...
    unfold mem_type_proj2_preserve.
    by repeat eexists.
  - by apply seg_type_proj2_preserve_trans.
  - move => a e a' HP HF.
    destruct a as [n1 m1]. destruct a' as [n2 m2].
    destruct HP as [m3 HP]. subst.
    destruct m1; inversion HF => //=; subst.
  ...
  - simpl in HPreserve. destruct fold_res. subst.
    ...
    inversion H1; inversion H2; subst; clear H1; clear H2.
    split => //.
    unfold seg_size, operations.seg_length => /=.
    by rewrite HLen.
Qed.
...
Proof.
  ...
  remove_bools_options.
  apply write_bytes_preserve_type in HStore; destruct HStore as (_ & H1 & H2); rewrite H1; rewrite H2.
  apply/andP; split => //=.
  apply N.leb_le.
  by lias.
Qed.

Lemma seg_extension_store: forall m h bs tlen seg,
    segstore m h bs tlen = Some seg ->
    seg_extension m seg.
Proof.
...
Lemma nth_error_size {A} : forall l i (x:A),
    List.nth_error l i = Some x ->
    List.nth_error l (S i) = None ->
    length l = (S i).
Proof.
  intros.
  assert (i < length l)%coq_nat; first apply List.nth_error_Some.
  by rewrite H.
  assert (S i >= length l)%coq_nat; first by apply List.nth_error_None.
  lias.
Qed.


Lemma nth_error_same_length {A} (l : list A) (l' : list A) :
  (forall i, List.nth_error l i = None <-> List.nth_error l' i = None) ->
  length l = length l'.
Proof.
  intro H.
  destruct l, l' => //=.
  assert (Some a = None); first by apply (H 0).
  done.
  assert (Some a = None); first by apply (H 0).
  done.
  destruct (List.nth_error (a :: l) (length l)) eqn:Hl; last first.
  apply List.nth_error_None in Hl.
  simpl in Hl. lias.
  destruct (List.nth_error (a0 :: l') (length l)) eqn:Hl'; last first.
  apply H in Hl'. by rewrite Hl' in Hl.
  destruct (List.nth_error (a :: l) (S (length l))) eqn:Hln.
  assert (List.nth_error (a :: l) (S (length l)) <> None); first by rewrite Hln.
  apply List.nth_error_Some in H0.
  simpl in H0. lias.
  apply H in Hln.
  rewrite <- (nth_error_size Hl' Hln).
  simpl.
  done.
Qed.
...
Proof.
  intros.
  unfold seg_extension.
  specialize (salloc_same_size H) as H'.
  apply/andP; split; last by inversion H; subst.
...
Proof.
  intros.
  unfold seg_extension.
  specialize (sfree_same_size H) as H'.
  apply/andP; split; last by inversion H; subst.
  unfold seg_size, operations.seg_length, seg_length.
  destruct m => /=.
  destruct seg_data => /=.
...
Lemma forall_all_agree: forall l,
    List.Forall all_agree l.
Proof.
  induction l => //.
...
Proof.
  ...
  rewrite -> List.Forall_forall in H.
  rewrite -> List.Forall_forall in H0.
  repeat split => //; remove_bools_options; simpl in HF; simpl in HT; subst.
  - rewrite -> List.Forall_forall. move => x HIN.
    apply H in HIN. unfold cl_type_check_single in HIN.
    destruct HIN.
    unfold cl_type_check_single.
...
Proof.
  ...
  rewrite N.add_0_l.
  exact H.
  apply N.lt_le_incl, N.sub_0_le in Hab.
  rewrite Hab.
  rewrite N.add_0_r.
  exact H2.
Qed.
...
Proof.
  ...
  inversion HAlloc; subst. unfold seg_agree in H.
  exact H.
Qed.
...
Lemma reduce_inst_unchanged: forall s f es me s' f' es',
    reduce s f es me s' f' es' ->
    f.(f_inst) = f'.(f_inst).
Proof.
  move => s f es me s' f' es' HReduce.
  induction HReduce => //=.
  induction H => //=.
Qed.


Lemma store_extension_reduce: forall s f es me s' f' es' C tf loc lab ret,
    reduce s f es me s' f' es' ->
...
Proof.
  move => s f es me s' f' es' C tf loc lab ret HReduce.
  ...
  - replace [::AI_const v; AI_basic (BI_set_global i)] with ([::AI_const v] ++ [::AI_basic (BI_set_global i)]) in HType => //=.
    ...
    + simpl. unfold store_extension => //=.
      ...
      -- eapply glob_extension_update_nth; eauto => //=.
         ...
         destruct v => //=; destruct g_val => //=.
         destruct n1 => //=; destruct n0 => //=.
  ...
  - (* update memory : store none *)
    ...
    apply AI_const2_typing in H5 as ->.
    apply et_to_bet in H6; last by split; eexists. apply Store_typing in H6.
  ...
  - (* store some *)
    ...
    destruct HType as [ts [t1s [t2s [t3s [H3 [H4 [H5 H6]]]]]]]. subst.
    apply AI_const2_typing in H5 as ->.
    apply et_to_bet in H6; last by eexists; eexists. 
  ...
  - (* grow_memory *)
    ...
    apply e_composition_typing in HType.
    destruct HType as [ts [t1s [t2s [t3s [H3 [H4 [H5 H6]]]]]]]. subst.
    apply AI_const_typing in H5 as ->.
    apply et_to_bet in H6; last by repeat eexists. 
  ...
  - (* segstore some *)
    ...
    apply AI_const2_typing in H15 as ->.
    apply et_to_bet in H17; auto_basic. apply Segstore_typing in H17.
    ...
    + split => //.
    eapply store_memory_extension_store_typed; eauto => //=.
    destruct s; destruct HST as (_&_&?&_) => //.
    remember HST as HST2. clear HeqHST2.
    unfold store_typing in HST.
    destruct s => //=.
  ...
  - (* segstore none? *)
    ...
    destruct HType as [ts [t1s [t2s [t3s [H13 [H14 [H15 H17]]]]]]]. subst.
    apply AI_const2_typing in H15 as ->.
    apply et_to_bet in H17; auto_basic. 
...
Lemma t_preservation_vs_type: forall s f es ms s' f' es' C C' lab ret t1s t2s,
    reduce s f es ms s' f' es' ->
    store_typing s ->
    store_typing s' ->
    inst_typing s f.(f_inst) C ->
    inst_typing s' f.(f_inst) C' ->
...
Proof.
  move => s f es ms s' f' es' C C' lab ret t1s t2s HReduce HST1 HST2 HIT1 HIT2 HType.
  generalize dependent t2s. generalize dependent t1s.
  ...
  - (* convert_et_to_bet. *)
    ...
    apply AI_const_typing in H4 as ->.
...
Lemma t_preservation_e: forall s f es me s' f' es' C t1s t2s lab ret,
    reduce s f es me s' f' es' ->
    store_typing s ->
    store_typing s' ->
    inst_typing s f.(f_inst) C ->
...
Proof.
  ...
  - (* Set_local *)
    ...
    apply AI_const_typing in H9 as ->.
    apply et_to_bet in H10; auto_basic. 
  ...
  - (* Set_Global *)
    ...
    destruct HType as [ts' [t1s' [t2s' [t3s' [H7 [H8 [H9 H10]]]]]]].
    apply AI_const_typing in H9 as ->. 
    apply et_to_bet in H10; auto_basic. apply Set_global_typing in H10.
    destruct H10 as [g [t [H4 [H5 [H6 [H11 H12]]]]]]. subst.
    apply ety_a'; auto_basic => //=.
    simpl in H12. simpl in H4. simpl.
    apply concat_cancel_last in H11. destruct H11. subst.
    apply bet_weakening_empty_both.
    by apply bet_empty.
  - (* Load None *)
    convert_et_to_bet.
    2:{ unfold VAL_int32 => /=. repeat (try split; eexists). }
    rewrite separate1 in HType. 
    ...
    rewrite catA.
    apply et_weakening_empty_1.
    apply ety_const'. apply typeof_deserialise. 
  - (* Load Some *)
    rewrite separate1 in HType. 
    apply e_composition_typing in HType.
    destruct HType as [ts' [t1s' [t2s' [t3s' [H7 [H8 [H9 H10]]]]]]].
  ...
  - (* Store None *)
    ...
    apply e_composition_typing in HType.
    destruct HType as [ts' [t1s' [t2s' [t3s' [H7 [H8 [H9 H10]]]]]]].
    fold_const_in (NVAL_int32 k) H9. apply AI_const2_typing in H9 as ->.
    convert_et_to_bet. 
  ...
  - (* Store Some *)
    ...
    apply e_composition_typing in HType.
    destruct HType as [ts' [t1s' [t2s' [t3s' [H7 [H8 [H9 H10]]]]]]].
    apply AI_const2_typing in H9 as ->.
    convert_et_to_bet. 
    apply Store_typing in H10.
    destruct H10 as [H11 [H12 H13]].
      apply concat_cancel_last_n in H11 => //.
  ...
  - (* Grow_memory fail *)
    ...
    apply e_composition_typing in HType.
    destruct HType as [ts' [t1s' [t2s' [t3s' [H7 [H8 [H9 H10]]]]]]].
    apply AI_const_typing in H9 as ->. convert_et_to_bet. 
    apply Grow_memory_typing in H10.
    destruct H10 as [ts [H11 [H12 H13]]]. subst.
    simpl.
    apply ety_a'; auto_basic.
  ...
  - (* Segload None *)
    ...
    destruct H10 as [ts  [H20 H13]].
    apply concat_cancel_last in H20. destruct H20. subst.
    rewrite catA.
    apply et_weakening_empty_1. apply ety_const'. apply typeof_deserialise. 
  - (* Segload Some *) 
    rewrite separate1 in HType. 
    apply e_composition_typing in HType.
    destruct HType as [ts' [t1s' [t2s' [t3s' [H12 [H15 [H16 H11]]]]]]].
    apply AI_const_typing in H16 as ->. convert_et_to_bet. 
  ...
  - (* Store None *)
    ...
    apply AI_const2_typing in H900 as ->. convert_et_to_bet. 
  ...
  - (* Store None *)
    ...
    apply AI_const2_typing in H9 as ->. convert_et_to_bet. 
    apply Segstore_typing in H1000 as H110 .
      apply concat_cancel_last_n in H110 => //.
      move/andP in H110. destruct H110. move/eqP in H0. subst.
      apply ety_a'; auto_basic => //=.
...
Theorem t_preservation: forall s f es me s' f' es' ts,
    reduce s f es me s' f' es' ->
    config_typing s f es ts ->
    config_typing s' f' es' ts.
Proof.
  move => s f es me s' f' es' ts HReduce HType.
