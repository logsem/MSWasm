...
From MSWasm Require Import stdpp_aux instantiation type_preservation datatypes memory_list.
From MSWasm.iris.instantiation Require Export instantiation_properties.
  ...
  Forall (fun '{| moddata_data := (Mk_memidx n);
            ...
            | Some doffi =>
              ...
              | Some (Mk_memidx k) =>
                ...
                | Some mi => (N.of_nat (nat_of_int doffi + length bs) <= operations.mem_length mi)%N
                | None => False
                end
              | _ => 
           ...
           ++ iDestruct "IHapply" as "(Hfc & Htc & Hmc & Hgc)".
              ...
              ** unfold import_func_wasm_check.
                 ...
                 --- iPureIntro.
                     ...
                     apply Forall2_cons; split => //=; first eexists cl => //.
...
Lemma import_mem_wasm_lookup v_imps t_imps wms ws :
  gen_heap_interp (gmap_of_memory (s_mems ws)) -∗
    gen_heap_interp (gmap_of_list (fmap operations.mem_length (s_mems ws))) -∗
...
Proof.
  ...
  replace (operations.mem_length mem) with (operations.mem_length m0) => //.
  unfold operations.mem_length, memory_list.mem_length.
  by rewrite Hmdata.
Qed.
...
Definition instantiation_resources_post_wasm m v_imps t_imps wfs wts wms wgs (idfstart: option nat) (inst: instance) : iProp Σ :=
  ...
  let v_imp_descs := map (fun mexp => mexp.(modexp_desc)) v_imps in
   ...
   (* The relevant initial values of allocated resources, as well as the newly
    ...
    module_inst_resources_wasm m inst tab_allocs mem_allocs glob_allocs. 
...
Lemma alloc_mem_state_update mems mems' nm:
  ...
  gen_heap_interp (gmap_of_list (fmap operations.mem_length mems)) -∗
...
Proof.
  ...
  (* table size *)
  specialize (gmap_of_list_append_big (operations.mem_length <$> mems) (operations.mem_length <$> nm)) as Hgmap.
  destruct Hgmap as [Hgunion Hgdisj].
  rewrite fmap_app.
  rewrite Hgunion.
...
Proof.
  ...
  unfold operations.mem_length, memory_list.mem_length in Hmemlength.
  ...
  replace (operations.mem_length <$> s_mems ws') with (operations.mem_length <$> s_mems ws); last first.
  {
    rewrite Hws' => /=.
    rewrite list_fmap_insert => /=.
    rewrite list_insert_id => //.
  ...
  unfold operations.mem_length, memory_list.mem_length in *.
  (* Now, update the contents *)
  destruct ws, ws' => /=.
  simpl in *.
...
Lemma init_mem_state_update (ws ws': store_record) (inst: instance) (d_off: N) (d: module_data) (m_ind: nat) (mem: memory) (d_pay: list byte) mem' :
  ...
  (d_off + N.of_nat (length d.(moddata_init)) <= operations.mem_length mem)%N ->
...
Proof.
  ...
  unfold operations.mem_length, memory_list.mem_length in Heboundcheck.
  rewrite -> Nat2N.id in *.
  ...
  - move: H.
    ...
    + destruct (update_mem _ _ _) eqn:Hupdmem => //=.
      ...
      { rewrite H0 in Hwmslookup.
        ...
        rewrite map_length.
        rewrite take_length drop_length.
        unfold operations.mem_length, memory_list.mem_length in H1.
        simpl in *.
        apply/andP; split => //.
...
Definition wms_bound_check (wms: gmap N memory) inst (d_offs : list N) datas :=
  all2 (fun off e => match inst.(inst_memory) !! (match e.(moddata_data) with | Mk_memidx i => i end) with
                  | Some i => match wms !! (N.of_nat i) with
                             | Some mi => (off + N.of_nat (length e.(moddata_init)) <=? operations.mem_length mi)%N
                             | None => false
                             end
                  | None => false
                  end)
                ...
                -- destruct (update_mem _ _ _) eqn: Hupdmem => //=.
                   ...
                   2:{ (* boundcheck *)
                     ...
                     ++ rewrite lookup_insert.
                        ...
                        replace (operations.mem_length m0) with (operations.mem_length m1) => //.
                        apply update_mem_length in Hupdmem.
                        unfold operations.mem_length, memory_list.mem_length.
                        by rewrite Hupdmem.
                     ++ rewrite lookup_insert_ne; last by lias.
