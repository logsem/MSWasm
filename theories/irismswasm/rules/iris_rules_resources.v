...
From MSWasm.iris.helpers Require Import iris_wasm_determ.
From MSWasm.iris.helpers.prelude Require Import iris_reduce_det_prelude.
From MSWasm Require Import handle operations memory_list common stdpp_aux opsem properties.
...
Lemma mem_block_lookup_data wms n mem:
  ...
  (gen_heap_interp (gmap_of_list (fmap operations.mem_length wms))) -∗
  n ↦[wmblock] mem -∗
...
Proof.
  ...
  - iIntros "!>" (es σ2 efs HStep).
    ...
    try by unfold first_instr in Hstart ; simpl in Hstart ; try rewrite first_instr_instr_const in Hstart; inversion Hstart.
...
Proof.
  iIntros "Hf Hwp".
  iApply wp_lift_step => //=. destruct v => //. 
  iIntros (σ ns κ κs nt) "Hσ".
  destruct σ as [[ws locs ] inst ].
  ...
  - iPureIntro.
    ...
    eexists [_],_,(_,_,_),_.
    repeat split => //=.
    apply rm_silent, r_simple, rs_tee_local; destruct v => //. 
  - iIntros "!>" (es σ2 efs HStep).
    iMod "Hfupd".
    iModIntro.
    destruct σ2 as [[ws' locs' ] inst' ] => //=.
...
Proof.
  ...
  - iIntros "!>" (es σ2 efs HStep) "!>".
    ...
    prim_split κ HStep H.
    iris_wp_def.only_one_reduction H. iFrame. destruct (g_val g); iFrame. 
Qed.
    ...
    + assert (j <= length bs) ; first lia.
      ...
      * assert (exists dat0, mem_update (n + off + N.of_nat (length bs - S j))%N
         ...
         { unfold mem_update. apply N.leb_le in Hlen.
           ...
           rewrite H2.
           unfold operations.mem_length, memory_list.mem_length in Hlen.
           lia.
...
Proof.
  ...
  destruct (k + (off + 1 + N.of_nat (length bs)) <=? operations.mem_length m)%N ; try by inversion Hm.
...
Proof.
  ...
  simplify_map_eq.
  destruct Hinstn; first subst.
  - destruct (inst_memory winst) eqn:Hinstmem => //. iSplit.
    + iPureIntro.
      destruct s => //=.
      unfold language.reducible, language.prim_step => /=.
    ...
    + iIntros "!>" (es σ2 efs HStep).
      ...
      eapply reduce_det in HStep as [H | [ (? & Hfirst & ?) | [ [? Hfirst] | (?&?&?& Hfirst & Hfirst2 &
                                                                                Hfirst3 & Hσ & Hme)]]] ;
        last (eapply rm_silent, r_load_failure => //= ; unfold smem_ind ; try by rewrite Hinstmem) ;
        try by     unfold first_instr in Hfirst ; simpl in Hfirst ; inversion Hfirst.
...
Proof.
  ...
  - iIntros "!>" (es σ2 efs HStep). 
    destruct σ2 as [[ws' locs'] inst'] => //=.
    prim_split κ HStep H.
    remember [AI_basic (BI_const (NVAL_int32 c)) ; AI_basic BI_grow_memory] as es0.
    ...
    { (* grow_memory failed *)
      ...
      iFrame => //. iModIntro.
      iFrame. iSimpl. iSplitL; last done. 
      iRight.
      by iFrame.  }
    rewrite Heqes0 in H0.
