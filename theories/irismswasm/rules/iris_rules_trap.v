...
From MSWasm.iris.rules Require Export iris_rules_control.
From MSWasm.iris.language Require Import iris_wp_def.
From MSWasm Require Import datatypes operations properties opsem.
From MSWasm.iris.helpers.prelude Require Import iris_reduce_properties iris_split_reduce.
From MSWasm.iris.helpers Require Import iris_prim_step_split.
...
Section trap_rules.
  ...
  Proof.
    ...
    { iPureIntro.
      ...
      destruct σ as [[ ??]?]. simpl.
      repeat split;eauto.
      eapply rm_silent, r_simple,rs_trap.
  ...
  Lemma wp_seq_trap (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) f f' Φ:
    □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) ∗
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es1 @ s; E {{ w, Φ w ∗ ↪[frame] f'}})
     ⊢ WP (es1 ++ es2) @ s; E {{ w, Φ w ∗ ↪[frame] f'}}.
  Proof.
    iIntros "(#HΦ & Hf & Hes1)".
    ...
    { eapply lfilled_to_val_app with (i:=0) (lh:=LH_base [] []) in Hetov as HH.
      ...
      iMod ("Hes1" with "Hf") as "[H Hf]". iFrame.
      iDestruct ("HΦ" with "H") as "%"; subst. 
    ...
    iMod ("Hes1" with "Hf") as "(H & Hf)". iDestruct ("HΦ" with "H") as "%". subst. 
    iApply fupd_frame_l.
    iSplit.
    { iPureIntro.
      destruct s => //.
  ...
  { destruct σ as [[ ??]?].
    ...
    - iIntros (e2 σ2 efs HStep).
      assert (exists me, κ = [me] /\ efs = []) as (me & -> & ->); first by apply prim_step_obs_efs_empty in HStep;  destruct HStep as [me HStep] ; inversion HStep; eexists.
      ...
      + move/lfilledP in Hlf1.
        ...
        iApply ("IH" with "[$Hf]").
        iIntros "Hf".
        iApply (wp_trap with "H");auto. 
  }
  Qed.

  Lemma wp_val_trap (s : stuckness) (E : coPset) v0 (es1 : language.expr wasm_lang) f f' Φ :
      □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) ∗
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es1 @ NotStuck; E {{ w, Φ w ∗ ↪[frame] f' }})
     ⊢ WP ((AI_const v0) :: es1) @ s; E {{ w, Φ w ∗ ↪[frame] f' }}.
  Proof.
      iIntros "(#HΦ & Hntrap & H)".
  ...
  { repeat rewrite wp_unfold /wp_pre /= Hes.
    iMod ("H"  with "Hntrap") as "(H & Hf)". iDestruct ("HΦ" with "H") as "%". subst.
  ...
  { repeat rewrite wp_unfold /wp_pre /= Hes.
    ...
    { apply lfilled_Ind_Equivalent in Hfill1.
      ...
      assert (iris.prim_step (vs0 ++ [AI_trap] ++ es'0)%SEQ σ2 [ME_empty] [AI_trap] σ2 []) as Hstep'.
  ...
  Lemma wp_val_app_trap' (s : stuckness) (E : coPset) vs (es : language.expr wasm_lang) f f' Φ :
      □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) ∗
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es @ NotStuck ; E {{ w, Φ w ∗ ↪[frame] f' }}%I)
     ⊢ WP ((v_to_e_list vs) ++ es) @ s ; E {{ w, Φ w ∗ ↪[frame] f' }}%I.
  Proof.
    iIntros "(#HΦ & Hf & HWP)".
    ...
    { 
      iSimpl.
      iApply wp_val_trap. 
      iFrame "HΦ Hf". iIntros "Hf".
      iApply ("IH" $! _ _ _ with "[Hf]") => //=.
    }
  Qed.
  
  Lemma wp_val_app_trap (s : stuckness) (E : coPset) vs v' (es : language.expr wasm_lang) f f' Φ :
    iris.to_val vs = Some (immV v') ->
          □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) ∗
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es @ NotStuck ; E {{ w, Φ w ∗ ↪[frame] f'}}%I)
     ⊢ WP (vs ++ es) @ s ; E {{ w, Φ w ∗ ↪[frame] f' }}%I.
  Proof.
    iIntros "%Hves (#HΦ & Hf & Hwp)".
    apply iris.of_to_val in Hves; subst.
    iApply wp_val_app_trap'.
    by iFrame.
  Qed.

  Lemma wp_label_trap s E LI vs n es' es'' f f' Φ :

    const_list vs ->
    □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) -∗
    ↪[frame] f -∗
    (↪[frame] f -∗ WP LI @ E {{ w, Φ w ∗  ↪[frame]f' }}) -∗
    WP vs ++ [AI_label n es' LI] ++ es'' @ s; E {{ w, Φ w ∗  ↪[frame]f' }}.
  Proof.
    iLöb as "IH" forall (vs LI n es' f f' es'' s).
    iIntros (Hconst) "#HΦ Hf Hcont".
    destruct (iris.to_val LI) eqn:He.
    { iDestruct ("Hcont" with "Hf") as "Hcont".
      iDestruct (wp_unfold with "Hcont") as "Hcont".
      rewrite /wp_pre /= He.
      iMod "Hcont" as "(H & Hf)". iDestruct ("HΦ" with "H") as "%". subst.
      apply to_val_trap_is_singleton in He as ->.
      apply const_list_to_val in Hconst as [v Hv].
      iApply wp_val_app_trap;eauto. iFrame "HΦ Hf".
      iIntros "Hf".
      rewrite separate1.
      iApply wp_seq_trap. iFrame "HΦ Hf". iIntros "Hf".
    ...
    { apply const_list_to_val in Hconst as [v Hv].
      ...
      { iPureIntro.
        ...
        destruct σ as [[ ? ?] ?].
        prim_split x H H. 
        eexists [_], [AI_label n es' x0],(_,_,_),[].
        split => //.
        eapply rm_label. exact H.
      ...
      { apply lfilled_Ind_Equivalent in Hfill'. inversion Hfill';subst.
        ...
        apply prim_step_obs_efs_empty in Hstep as Heq'; destruct Heq' as [? Heq']; simplify_eq.
        iSpecialize ("H" $! _ _ _ Hstep').
        repeat (iMod "H"; iModIntro; try iNext).
        destruct σ2 as [[ ??]?].
        iDestruct ("H") as "[$ H]".
        ...
        iDestruct ("IH" $! [] _ _ es' _ _ [] with "[] HΦ Hf H") as "H";auto. }
      { destruct σ2 as [[ ? ?] ?].
        set (σ2 := (s0,l,i)).
  ...
  Lemma reduce_det_local_trap ws f me ws' f' es2 n f0 :
    opsem.reduce ws f [AI_local n f0 [AI_trap]] me ws' f' es2 ->
    es2 = [AI_trap] ∧ ws = ws' ∧ f = f' /\ me = ME_empty.
  Proof.
  ...
  Proof.
    ...
    - iPureIntro.
      ...
      unfold iris.prim_step => /=.
      repeat split => //. apply rm_silent, r_simple. apply rs_local_trap; auto.
    - destruct σ as [[ws locs] inst].
      iIntros "!>" (es2 σ2 efs HStep) "!>".
  ...
  Lemma wp_frame_trap_nested (s : stuckness) (E : coPset) n f f0 LI f' Φ :

    iris.to_val [AI_local n f LI] = None ->
    □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) -∗
      ↪[frame] f0 -∗
     (↪[frame] f -∗ WP LI @ s; E {{ w, Φ w ∗ ↪[frame] f' }}) -∗
     WP LI @ s; E FRAME n; f {{ w, Φ w ∗ ↪[frame] f0 }}.
  Proof.
    iIntros (Hnone) "#HΦ Hframe H".
    rewrite wp_frame_rewrite.
    iLöb as "IH"
  forall (s E LI f f' f0 Hnone).
    ...
    { iApply wp_unfold. unfold wp_pre. simpl. rewrite Hnone.
      ...
      { iPureIntro. destruct s => //.
        ...
        destruct Hred as [? [? [? ?]]].
        destruct x1 as [[?  ?] ?].
        destruct H as [? H]. prim_split x H H.
        
        eexists [_], [AI_local n {| f_locs := l0; f_inst := i0 |} x0],(_,_,_),[].
        split => //. eapply rm_local. eauto. }
      iIntros (e2 σ2 efs Hstep).
      destruct σ2 as [[ws' locs'] inst'].
      prim_split κ Hstep H.
      ...
      { iFrame.
        ...
        rewrite /wp_pre /= HLI.
        iMod "Hx" as "[H Hf]". iDestruct ("HΦ" with "H") as "%". done.
      }
  ...
  Lemma wp_seq_trap_ctx (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) f f' i lh Φ :
      □ (∀ w, Φ w -∗ ⌜ w = trapV ⌝) ∗
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es1 @ NotStuck; E {{ w, Φ w  ∗ ↪[frame] f' }})
     ⊢ WP (es1 ++ es2) @ s; E CTX i; lh {{ w, Φ w ∗ ↪[frame] f' }}.
  Proof.
    iIntros "(#HΦ & Hf & Hes1)".
    ...
    { iIntros (LI Hfilled).
      ...
      erewrite app_assoc.
      iApply (wp_seq_trap with "[Hf Hes1]"). iFrame "HΦ Hf".
      iIntros "Hf".
  ...
  Proof.
    iIntros (Hconst) "Hf". rewrite app_assoc. iApply wp_seq_trap_ctx.
    iSplitR; first by iIntros "!>" (w) "->". 
    iFrame. iIntros "Hf".
...
{ iIntros "[Hntrap [Ht [Hf [Hes1 Hes2]]]]".
  ...
  }
  {
    ...
    { iApply wp_unfold. rewrite /wp_pre /= Hetov.
      ...
      - iIntros (e2 σ2 efs HStep').
        ...
        apply prim_step_obs_efs_empty in HStep' as Hemp. destruct Hemp as [? Hemp]. inversion Hemp;subst;clear Hemp.
  ...
  { iApply wp_unfold.
    ...
    - iIntros (es2 σ2 efs HStep).
      ...
      assert (exists me, κ = [me] /\ efs = []) as (me & -> & ->); first by apply prim_step_obs_efs_empty in HStep; destruct HStep as [? HStep]; inversion HStep; eexists.
