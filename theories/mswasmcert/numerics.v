...
Definition canonical_pl := ZArith.Zpower.shift_pos (Z.to_pos prec - 2) 1.
...
Definition ZofB_param (divP divN : Z -> Z -> Z) (z : T) :=
  ...
  | Binary.B754_finite s m 0%Z _ =>
    Some (Zaux.cond_Zopp s (Z.pos m))
  | Binary.B754_finite s m (Z.pos e) _ =>
    Some (Zaux.cond_Zopp s (Z.pos m) * Z.pow_pos (Zaux.radix_val Zaux.radix2) e)%Z
  | Binary.B754_finite s m (Z.neg e) _ =>
    let: div := if s then divN else divP in
    Some (Zaux.cond_Zopp s (div (Z.pos m) (Z.pow_pos (Zaux.radix_val Zaux.radix2) e)))
  | _ => None
  end.
