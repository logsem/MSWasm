...
From MSWasm Require Import operations segment_list handle.
...
| bet_reinterpret : forall C t1 t2, t1 <> t2 -> t1 <> T_handle -> t2 <> T_handle -> Nat.eqb (t_length t1) (t_length t2) ->
...
Definition seg_agree (s : segment) : Prop :=
  match (seg_max_opt s) with
  | None => True
  | Some n => N.le (seg_size s) n
  end.

Definition all_agree (a : allocator) : Prop :=
  True.

Definition store_typing (s : store_record) : Prop :=
  match s with
  | Build_store_record fs tclss mss segs alls gs =>
