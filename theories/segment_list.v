...
(* Lemma decision_equal_first nid :
  forall (x: N * N * N), Decision (let '(n, _, _) := x in n = nid).
Proof. intros [[??]?]. unfold Decision. decidable_equality. Qed. *)

(*Definition find (nid : N) (l : list (N * N * N)) : option (N * N) :=
  match partition (fun '(n, _, _) => (n =? nid)%N) l with
  | ([ (_,b,c) ], _) => Some (b, c)
  | _ => None
  end.


Definition find_and_remove (nid : N) (l : list (N * N * N)): option (list (N * N * N) * N * N) :=
  match partition (fun '(n, _, _) => (n =? nid)%N) l with 
  | ([ (_,b,c) ], l) => Some (l, b, c)
  | _ => None
  end. *)
...
Proof.
  ...
  apply IHl in H0.
  unfold isFree in H0. unfold find in H0. simpl in H0.
  destruct (partition _ l) eqn:Hl. done. 
...
(*
Lemma partition_all_true {A} f (l: list A): forall l', partition f l = (l', []) -> l = l'.
Proof. induction l => //=. intros * H; by inversion H.
       destruct (partition f l). destruct (f a) => //.
       intros * H; inversion H; subst. by erewrite IHl. Qed.

Lemma partition_all_false {A} f (l: list A): forall l', partition f l = ([], l') -> l = l'.
Proof. induction l => //=. intros * H; by inversion H.
       destruct (partition f l). destruct (f a) => //.
       intros * H; inversion H; subst. by erewrite IHl. Qed.
  *)    
...
(*  induction l => //=.
  ...
  unfold find_and_remove in Hfr. simpl in Hfr.
  destruct (partition _ l) eqn:Hl.
  destruct (n0 =? nid)%N.
  - destruct l0; inversion Hfr; subst.
    apply partition_all_false in Hl by destruct Hcomp.
