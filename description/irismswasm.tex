...
We will later introduce a more general notion of context and a stronger frame rule.
...
Lastly, let us mention local and global variables. Access to both of these is done through integer indexes \( i \) rather than variable names. To explain where their values are stored, let us take a closer look at the frame \( F \) that forms the state of a \wasm execution:
...
\wasm is somewhat unusual as an assembly-like language in that it features no general jump instruction. The only control flow operations allowed are direct or indirect function calls, and \emph{break} (or \emph{branch}) instructions \( \xxWbr{i} \) which allows to exit a group of instructions enclosed in a \KK{block}, \KK{loop} or \KK{if} instruction. Let us first discuss the later.

The instruction \( \xxWif~{\V{tf}}~{\V{bs}_t}~{\V{bs}_f} \) takes one argument from the stack and desugars to either \( \xxWblock~{\V{tf}}~{\V{bs}_t} \) or \( \xxWblock~{\V{tf}}~{\V{bs}_f} \) depending on whether the value was 0. Let us have a closer look at the instructions \( \xxWblock~{\V{tf}}~{\V{bs}} \) and \( \xxWloop~{\V{tf}}~{\V{bs}} \). Here, \V{bs} is the content of the \KK{block} or \KK{loop}, and \V{tf} is the type signature, i.e. \( \V{tf} = \V{t1s}\to\V{t2s} \) where \( \V{t1s} \) is the list of the types that are expected to be on the stack upon entering the \KK{block} or \KK{loop}, and \( \V{t2s} \) is the list of the types that will be on the stack upon exiting. The exact contents of these lists are only useful for typing, and the operational semantics cares only about the length of these lists.
...
Finally, let us describe the way function calls work in \wasm. Function calls can be to a statically know function with the \xxWcall instruction, or to a dynamically determined function with the \( \KK{call\_indirect} \) instruction. In the first case, the immediate argument \V{i} of instruction \( \xxWcall~\V{i} \) refers to the index of the function to be called, were if a module imports \( k \) functions and defines \( \ell \) ones, the imported functions have indices 0 to \( k - 1 \) and the functions defined in the module have indices \( k \) to \( k + \ell - 1 \), in the order in which they appear. Just like the values of global variables, the closures for the functions are all in the store \( S \) together with the closures for functions defined elsewhere, and the frame \( F \) keeps track of the indirection.
...
or if a \xxWreturn instruction is met. In that case, the subscript \( n \) indicated how many values to take from the stack before exiting. Unlike for break instructions, a \xxWreturn instruction should always target the closest \KK{local}, and thus does not need to take an immediate argument:
\[ \inferrule{|\V{vs}|=n}{(S,F,[\xxWlocal{n}{F_1}{\V{lh}_k(\V{vs}\cat\xxWreturn)}])\hookrightarrow(S,F,\V{vs})} \]

When reducing a native call, the code of the closure is placed inside a \KK{local} and inside a \xxWblock, meaning one use a \( \xxWbr{i} \) instruction with a carefully chosen \( i \) instead of a \xxWreturn instruction.
...
\subsection{Coarse-grain encapsulation and module instantiation via host language}
\label{sec:host}

\wasm code is organised in \emph{modules}, which have been formally shown~\cite{iriswasm} to provide encapsulation, as a module can only interact with another module's state via what that module explicitely exports.
...
To keep track of live handles, we use an \emph{allocator}. \citet{mswasm} do not go into much detail about how this allocator should be implemented, only stating it should allow for the definition of allocation and freeing functions that have some propertes which they only define intuitively. When implementing \irismswasm, our implementation defines the allocator as an asosciative map from the space of handle \xxWid{}s to pairs of integers representing the handle's original base address and bound.

This means allocating and freeing simply mean adding and removing from this map, and these operations can be performed with the \xxWsegalloc and \xxWsegfree instructions. Basing ourselves on \citet{mswasm}, we impose several conditions on the handle to be freed, like its base to be the original address the handle got allocated as (i.e. it cannot have been sliced, see next paragraph) and its offset to be zero. As for allocation, \irismswasm introduces a second operational semantics rule for \xxWsegalloc, allowing the allocation to randomly fail to represent realistic machine behaviour.
