...
From MSWasm.iris.examples.segstack Require Export segstack_common segstack_is_empty segstack_pop.
From MSWasm.iris.rules Require Import proofmode iris_rules.
From MSWasm.iris.logrel Require Import iris_fundamental iris_fundamental_get_local iris_fundamental_segstore.
From MSWasm Require Import type_checker_reflects_typing.
...
From MSWasm Require Import opsem stdpp_aux common properties.
 ...
 destruct (handle_addr h - base' + N.of_nat (t_length T_i32) <=? addr)%N eqn:Hlo.
  ...
  -- (* Case 1: the place where we wrote to memory is before addr *)
    ...
    ++ iRight.
       ...
       rewrite (length_bits _ T_i32) => //.
       rewrite Nat.add_comm Nat.sub_add. done. lia. 
