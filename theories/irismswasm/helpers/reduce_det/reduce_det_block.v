...
Lemma block_det vs n m t1s t2s s f me es s' f' es' :
  ...
  length t1s = n ->
  length t2s = m ->
  reduce s f (vs ++ [AI_basic (BI_block (Tf t1s t2s) es)]) me s' f' es' ->
  reduce_det_goal ME_empty s f [AI_label m [] (vs ++ to_e_list es)] me s' f' es' (vs ++ [AI_basic (BI_block (Tf t1s t2s) es)]).
Proof.
