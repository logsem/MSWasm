...
From MSWasm.iris.language Require Import iris iris_locations iris_atomicity iris_wp_def.
From MSWasm.iris.helpers Require Import iris_properties.
From MSWasm Require Import stdpp_aux datatypes operations properties opsem typing type_checker_reflects_typing.
...
Notation "{{{ P }}} es {{{ v , Q }}}" :=
  ...
  Definition buffer_program :=
    [
      ...
      BI_get_local local_h;
      BI_segload T_i32;
      BI_set_global global_result (* In order to later observe the return value, we store it in a global variable that the host code can examine (omitted in paper for simplicity) *)
    ].
