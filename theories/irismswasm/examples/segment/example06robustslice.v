...
*)
  ...
  Proof.
    ...
    iDestruct (big_sepL_app with "Hs") as "[Hsecret Hpublic]".
    ...
    fold_const. iApply (wp_wand with "[-]"). iApply (wp_set_global with "[] Hf Hg'").
    done.
    by instantiate (1 := λ x, ⌜ x = immV _ ⌝%I).
    iIntros (w) "[-> H]".
    instantiate (1 := λ x, (⌜ x = immV _ ⌝ ∗ _)%I).
    ...
    iSimpl.
    iApply fupd_wp.
    remember {| base := base h + 4; offset := offset h; bound := 4; handle.valid := handle.valid h; id := id h|} as h0.
    ...
    rewrite (separate2 (AI_handle _)).
      ...
      subst h0 => /=. iApply (wp_wand with "Hl").
    ...
    2: by iIntros "[% _]".
    iIntros (w) "[-> Hf]".
    iSimpl. iApply (wp_wand with "[Hf Ha Hs Htok]").
    unfold handle_addr. rewrite Hoff N.add_0_r.
    simpl in Hbound. rewrite - Hbound.
    iApply fupd_wp.
    iMod (cinv_cancel with "Hinv Htok") as "H"; first solve_ndisj.
    iDestruct "H" as (tbs) "(>%Htbs & >Hs' & Hhandles)".

    rewrite - (N2Nat.id base'). 
    iDestruct (wss_select (N.to_nat base') (N.to_nat bound') (N.to_nat (base h) + 4) 4 with "Hs'") as "(%Hlen & Hs' & Hrestitute
)".
    simpl in Hbase'. lia.
    simpl in Hbound'. lia.
    done.
    iDestruct big_sepL_app as "[_ H]".
    iDestruct ("H" with "[$Hs Hs']") as "Hs"; iClear "H".
    iApply (big_sepL_impl with "Hs'").
    iIntros "!>" (k0 x Hk) "Hs".
    rewrite map_length (length_bits _ T_i32); last done.
    iSimpl.
    replace (N.of_nat (N.to_nat (N.of_nat (N.to_nat (base h) + 4)) + k0))
      with (N.of_nat (N.to_nat (base h) + S (S (S (S k0))))); first done.
    lia. 
    
    iApply (wp_segfree with "[$Hf $Ha $Hs]") => //.
    rewrite app_length map_length (length_bits _ T_i32); last done.
    rewrite Hlen. 
    by rewrite Hbound.
    ...
    iAssert (∃ all1, interp_allocator all1)%I with "[Ha' Hrestore Hbl]" as "Hall".
    { iExists {| allocated := <[ id h := None ]> (allocated all0); next_free := next_free all0 |}.
      iExists _. iFrame.
      iApply (big_sepM_insert with "[Ha' Hrestore]"); first by rewrite lookup_delete.
      iSplitL "Ha'". 
      - iExists None. iFrame. iPureIntro. simpl.
        by rewrite lookup_insert.
      - iApply (big_sepM_impl with "Hrestore").
        iIntros "!>" (k0 [[[??]?]?] Hk0) "(%y1 & %Hy1 & Hall & Hy1)".
        iExists y1. iFrame. iPureIntro. simpl.
        rewrite lookup_insert_ne => //. intros <-.
        simpl in Hk0. rewrite lookup_delete in Hk0 => //. } 
 ...
 Ltac unfold_irwt_all :=
  ...
  Proof. 
    ...
    { apply Forall2_cons. split;auto. cbn.
      ...
      rewrite (separate8 (BI_segstore _)).
      eapply bet_composition'.
      { apply/b_e_type_checker_reflects_typing => /= ; by apply/eqP. }
      { apply/b_e_type_checker_reflects_typing => /= ; by apply/eqP. }
    } 
