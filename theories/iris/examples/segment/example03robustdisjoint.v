...
*)
  ...
  Proof.
    ...
    { rewrite fixpoint_interp_value_handle_eq.
                                              ...
                                              ⌜ ((base h0 + addr) `mod` handle_size)%N = 0%N /\
                                                (addr + handle_size <= bound h0)%N⌝ -∗
                                                  ⌜ ∃ addr' b,  (addr' < handle_size)%N /\
                                                                  tbs !! N.to_nat (addr + addr') = Some (b, Numeric)⌝ ∨ interp_value_handle (VAL_handle (handle_of_bytes (map fst (take (N.to_nat handle_size) (drop (N.to_nat addr) tbs))))))))%I with "[Hss]" as "HP".
      iNext. iExists _. iFrame. iSplit; first iPureIntro.
      ...
      assert (N.to_nat addr < N.to_nat (bound h)).
      rewrite - Hbound0.
      assert (H:=hsnz). rewrite nat_bin in H. lia.
      edestruct (repeat_lookup (T:= Integers.Byte.int * btag) (#00%byte, Numeric)) as [Hrl _].
      ...
      { destruct (f1 !! (id h0)) as [[[[??]?]?] |] eqn:Hf1 ; last done.
      ...
      iExists (allocator_insert (id h0) (base h0) (bound h0) all). iExists (<[ id h0 := _]> f1).
      iFrame "Hblack".
      iApply big_sepM_insert; first done.
      iSplitR "Hmap".
      iExists (Some (_,_)). iFrame. repeat iSplitL => //.
      iPureIntro. unfold allocator_insert => /=. by rewrite lookup_insert Hbound0.
    ...
    rewrite map_length (length_bits _ T_i32) => //.
    rewrite Hbound => //. 
