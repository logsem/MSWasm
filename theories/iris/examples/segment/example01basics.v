...
Notation "{{{ P }}} es {{{ v , Q }}}" :=
  ...
  Proof.
    ...
    iSimpl.
(*    all: rewrite (separate2 (AI_handle _)).
    all: iApply wp_seq.
    all: iSplitR; last first.
    1,3: iSplitL "Hf".
    1,3: fold_const.
    1,2: iApply (wp_tee_local with "Hf").
    1,2: iIntros "!> Hf".
    1,2: rewrite (separate1 (AI_const _)).
    1,2: iApply (wp_wand with "[-]").
    1,3: iApply wp_val_app => //.
    1,2: iSplitR; last first.
    1,3: iApply (wp_wand with "[-]"). 
    1,3: iApply (wp_set_local with "[] Hf").
    1,3: lia.
    1,2: by instantiate (1 := λ x, ⌜ x = immV [] ⌝%I).
    1,2: iIntros (v) "[-> H]".
    1,2: iSimpl.
    1,2: instantiate (1 := λ x, (⌜ x = immV _ ⌝ ∗ _)%I).
    1,2: iSplit; [done | iExact "H"].
    1,2: by iIntros "!> [% _]".
    1,2: iIntros (v) "[-> Hf]".
    1,2: instantiate (1 := λ x, (⌜ x = immV _ ⌝ ∗ _)%I).
    1,2: iSplit; [done | iExact "Hf"].
    3,4: by iIntros "[% _]".
    all: iIntros (w) "[-> Hf]".
    all: iSimpl.
    all: rewrite (separate3 (AI_handle _)).
    { iApply (wp_wand with "[Hf]"). 
