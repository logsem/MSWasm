...
Section fundamental.
  ...
  Proof.
    ...
    { (* it is a packed store *)
      ...
      { apply N.ltb_ge in Hlt'. iDestruct (mem_extract_insert _ (Wasm_int.N_of_uint i32m z + off) (tp_length p) with "Hmem") as (bv) "[Ha [Hmem %Hlenbv]]";[destruct p;simpl;lia|lia|].
        iApply wp_fupd.
        ...
        { iNext. unfold mem_block.
          ...
          iFrame. unfold operations.mem_length, memory_list.mem_length. iSimpl.
          rewrite -update_list_range_length;[iFrame|].
          { rewrite length_bytes_takefill. simpl in Hlt'.
            unfold operations.mem_length, memory_list.mem_length in Hlt'. lia. }
        }
        iModIntro.
    ...
    { destruct (N.ltb (operations.mem_length ms) ((Wasm_int.N_of_uint i32m z) + off + (N.of_nat (t_length t))))%N eqn:Hlt'.
      ...
      { apply N.ltb_ge in Hlt'. iDestruct (mem_extract_insert _ (Wasm_int.N_of_uint i32m z + off) (t_length t) with "Hmem") as (bv) "[Ha [Hmem %Hlenbv]]";[destruct t;simpl; try lia; try apply hsnz|lia|].
        iApply wp_fupd.
        iApply (wp_wand _ _ _ (λ vs, (⌜vs = immV _⌝ ∗ _) ∗ _)%I with "[Ha Hf]").
        ...
        { iNext. unfold mem_block.
          ...
          iFrame. unfold operations.mem_length, memory_list.mem_length. iSimpl.
          rewrite -update_list_range_length;[iFrame|].
          { erewrite length_bits;[|eauto]. simpl in Hlt'.
            unfold operations.mem_length, memory_list.mem_length in Hlt'. lia. }
        }
        iModIntro.
        iSplitR;[iLeft; iRight|iExists _;iFrame;iExists _;eauto].
