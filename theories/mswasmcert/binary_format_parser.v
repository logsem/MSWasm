...
From MSWasm Require Import datatypes datatypes_properties typing operations segment_list handle.
From compcert Require Import Integers.
From parseque Require Import Parseque.
From Coq.Strings Require Import Byte.
From MSWasm Require Import leb128.
...
Definition parse_handle_load {n} : byte_parser basic_instruction n :=
  exact_byte xc0 &> (uncurry (BI_load T_handle None) <$> parse_memarg).
...
Definition parse_handle_store {n} : byte_parser basic_instruction n :=
  exact_byte xc6 &> (uncurry (BI_store T_handle None) <$> parse_memarg).
...
Definition parse_i32_segload {n} : byte_parser basic_instruction n :=
  exact_byte xc1 &> (exact_byte x00 $> BI_segload T_i32).

Definition parse_i64_segload {n} : byte_parser basic_instruction n :=
  exact_byte xc2 &> (exact_byte x00 $> BI_segload T_i64).

Definition parse_f32_segload {n} : byte_parser basic_instruction n :=
  exact_byte xc3 &> (exact_byte x00 $> BI_segload T_f32).

Definition parse_f64_segload {n} : byte_parser basic_instruction n :=
  exact_byte xc4 &> (exact_byte x00 $> BI_segload T_f64).

Definition parse_handle_segload {n} : byte_parser basic_instruction n :=
  exact_byte xc5 &> (exact_byte x00 $> BI_segload T_handle).


Definition parse_i32_segstore {n} : byte_parser basic_instruction n :=
  exact_byte xc7 &> (exact_byte x00 $> BI_segstore T_i32).

Definition parse_i64_segstore {n} : byte_parser basic_instruction n :=
  exact_byte xc8 &> (exact_byte x00 $> BI_segstore T_i64).

Definition parse_f32_segstore {n} : byte_parser basic_instruction n :=
  exact_byte xc9 &> (exact_byte x00 $> BI_segstore T_f32).

Definition parse_f64_segstore {n} : byte_parser basic_instruction n :=
  exact_byte xca &> (exact_byte x00 $> BI_segstore T_f64).

Definition parse_handle_segstore {n} : byte_parser basic_instruction n :=
  exact_byte xcb &> (exact_byte x00 $> BI_segstore T_handle).

Definition parse_slice {n} : byte_parser basic_instruction n :=
  exact_byte xcc &> (exact_byte x00 $> BI_slice).


Definition parse_segalloc {n} : byte_parser basic_instruction n :=
  exact_byte xcd &> (exact_byte x00 $> BI_segfree).
...
Definition parse_segment_instruction {n} : byte_parser basic_instruction n :=
  parse_i32_segload <|>
    parse_i64_segload <|>
    parse_f32_segload <|>
    parse_f64_segload <|>
    parse_handle_segload <|>
    parse_i32_segstore <|>
    parse_i64_segstore <|>
    parse_f32_segstore <|>
    parse_f64_segstore <|>
    parse_handle_segstore.

Definition parse_handle_instruction {n} : byte_parser basic_instruction n :=
  parse_slice <|>
    parse_segalloc <|>
    parse_handleadd <|>
    parse_getoffset <|>
...
Definition language : [ Language ] := Fix Language (fun k rec =>
  ...
  let parse_be :=
    ...
    parse_segment_instruction <|>
    parse_handle_instruction <|>
...
Definition parse_numeric_byte {n} : byte_parser (byte * btag) n :=
  (fun b => (b, Numeric)) <$> anyTok.

Definition parse_handle_byte {n} : byte_parser (byte * btag) n :=
  (fun b => (b, Handle)) <$> anyTok.

Definition parse_tagged_byte {n} : byte_parser (byte * btag) n :=
...
#[export] Instance tokBytes : Tokenizer byte := MkTokenizer (fun x => x).
