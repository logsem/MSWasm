...
Definition parse_handle_load {n} : byte_parser basic_instruction n :=
  exact_byte xc0 &> (prod_curry (BI_load T_handle None) <$> parse_memarg).
...
Definition parse_handle_store {n} : byte_parser basic_instruction n :=
  exact_byte xc6 &> (prod_curry (BI_store T_handle None) <$> parse_memarg).
...
Definition parse_segsec_wrapper {n} : byte_parser parsing_module n :=
  (fun segs =>
     {|
       ...
       pmod_code := nil;
     |}) <$>
    (parse_with_customsec_star_before parse_segsec).

Definition parse_allsec_wrapper {n} : byte_parser parsing_module n :=
  (fun segs =>
     {|
       pmod_types := nil;
       pmod_funcs := nil;
