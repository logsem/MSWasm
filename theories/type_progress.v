...
Lemma reduce_composition: forall s cs f es me es0 s' f' es',
    const_list cs ->
    reduce s f es me s' f' es' ->
    reduce s f (cs ++ es ++ es0) me s' f' (cs ++ es' ++ es0).
Proof.
  move => s cs f es me es0 s' f' es' HConst HReduce.
  eapply rm_label; eauto; apply/lfilledP.
  - instantiate (1 := (LH_base cs es0)). instantiate (1 := 0).
    by apply LfilledBase.
  - by apply LfilledBase.
Qed.

Lemma reduce_composition_right: forall s f es me es0 s' f' es',
    reduce s f es me s' f' es' ->
    reduce s f (es ++ es0) me s' f' (es' ++ es0).
Proof.
  move => s f es me es0 s' f' es' HReduce.
...
Lemma reduce_composition_left: forall s cs f es me s' f' es',
    const_list cs ->
    reduce s f es me s' f' es' ->
    reduce s f (cs ++ es) me s' f' (cs ++ es').
Proof.
  move => s f es me es0 s' f' es' HConst HReduce.
...
Proof.
  ...
  remember H5 as H500. clear HeqH500.
...
Proof.
  ...
  destruct tc_local => //=. destruct tc_label => //=. destruct tc_return => //=.
  remove_bools_options.
  remember H4 as H500. clear HeqH500.
  ...
  unfold globals_agree in H500.
  eapply all2_projection in H500; eauto.
  remove_bools_options.
  by move/ltP in H6.
Qed.
...
Proof.
  ...
  apply H7 in HIN1. destruct HIN1 as [HIN1 _].
...
Ltac solve_progress_cont cont :=
  repeat eexists;
  solve [
      apply rm_silent; apply r_simple; solve [ eauto | constructor; eauto | cont; eauto ]
    | cont ].
...
Lemma t_progress_be: forall C bes ts1 ts2 vcs lab ret s f,
    ...
    exists me s' f' es', reduce s f (v_to_e_list vcs ++ to_e_list bes) me s' f' es'.
Proof.
  ...
  - (* Unreachable *)
    ...
    apply rm_silent, r_simple. by constructor.
  - (* Nop *)
    right.
    exists ME_empty, s, f, (v_to_e_list vcs ++ [::]).
    apply reduce_composition_left; first by apply v_to_e_is_const_list.
    apply rm_silent, r_simple. by constructor.
  - (* Drop *)
  ...
  - (* Block *)
    ...
    apply rm_silent, r_simple. eapply rs_block; eauto.
  ...
  - (* Loop *)
    right.
    exists ME_empty, s, f, [::AI_label (length vcs) [::AI_basic (BI_loop (Tf ts1 ts2) es)] (v_to_e_list vcs ++ to_e_list es)].
    apply rm_silent, r_simple. rewrite HConstType.
  ...
  - (* BI_br_table *)
    ...
    + remember HLength as H8. clear HeqH8.
      ...
      exists ME_empty, s, f, ((v_to_e_list (take (size t1s) vcs) ++ v_to_e_list (take (size ts) (drop (size t1s) vcs))) ++ [::AI_basic (BI_br n)]).
      apply reduce_composition_left.
      { by apply const_list_concat; apply v_to_e_is_const_list. }
      apply rm_silent, r_simple. apply rs_br_table => //.
      by lias.
    + assert (Inf : length ins <= Wasm_int.nat_of_uint i32m s0); first by lias.
      ...
      exists ME_empty, s, f, ((v_to_e_list (take (size t1s) vcs) ++ v_to_e_list (take (size ts) (drop (size t1s) vcs))) ++ [::AI_basic (BI_br i)]).
      apply reduce_composition_left.
      { by apply const_list_concat; apply v_to_e_is_const_list. }
      apply rm_silent, r_simple. apply rs_br_table_length => //.
      by lias.
  - (* Return *)
    subst.
    exfalso.
  ...
  - (* Call *)
    ...
    exists ME_empty, s, f, (v_to_e_list vcs ++ [:: (AI_invoke a)]).
    apply reduce_composition_left; first by apply v_to_e_is_const_list.
    by apply rm_silent, r_call => //. 
  - (* Call_indirect *)
    right.
    simpl in *.
    ...
    + (* Some a *)
      ...
      * exists (v_to_e_list (take (size t1s) vcs) ++ [::AI_invoke a]).
        apply reduce_composition_left; first by apply v_to_e_is_const_list.
        simpl.
        by eapply rm_silent, r_call_indirect_success; eauto.
      * exists (v_to_e_list (take (size t1s) vcs) ++ [::AI_trap]).
        apply reduce_composition_left; first by apply v_to_e_is_const_list.
        by eapply rm_silent, r_call_indirect_failure1; eauto.
    + (* None *)
      exists (v_to_e_list (take (size t1s) vcs) ++ [::AI_trap]).
      apply reduce_composition_left; first by apply v_to_e_is_const_list.
      by apply rm_silent, r_call_indirect_failure2.
  ...
  - (* Set_local *)
    ...
    exists ME_empty, s, (Build_frame (set_nth v f.(f_locs) i v) f.(f_inst)), [::].
    by eapply rm_silent, r_set_local; eauto.
  ...
  - (* Set_global *)
    ...
    + exists ME_empty, s0, f, [::].
      by apply rm_silent, r_set_global.
  ...
  - (* Segload *)
    ...
    repeat eexists _; eapply rm_segload_failure; eauto; try lias.
    repeat right. split => //. intro H; rewrite H in Hallign; done.
    destruct (segload s0 h (t_length t)) eqn:HLoadResult.
    repeat eexists.
  ...
  - (* Store *)
    ...
    + (* Store Some *)
      ...
      * exists ME_empty, (upd_s_mem s (update_list_at s.(s_mems) n m0)), f, [::].
        eapply rm_silent, r_store_packed_success; eauto.
        by unfold types_agree; apply/eqP.
      * exists ME_empty, s, f, [::AI_trap].
        eapply rm_silent, r_store_packed_failure; eauto.
        by unfold types_agree; apply/eqP.
    + (* Store None *)
      simpl in H0.
      destruct (store m (Wasm_int.N_of_uint i32m s0) off (bits v0) (t_length (typeof v0))) eqn:HStoreResult.
      * exists ME_empty, (upd_s_mem s (update_list_at s.(s_mems) n m0)), f, [::].
        eapply rm_silent, r_store_success; eauto.
        by unfold types_agree; apply/eqP.
      * exists ME_empty, s, f, [::AI_trap].
        eapply rm_silent, r_store_failure; eauto.
        by unfold types_agree; apply/eqP.
 ...
 - (* Segstore *)
    ...
    destruct (nat_of_bin h.(offset) + t_length (typeof v0) <=? nat_of_bin h.(bound)) eqn:Hhi;
      [ apply Nat.leb_le in Hhi | apply Nat.leb_gt in Hhi].
    destruct (find h.(id) a.(allocated)) eqn:Halloc.
    destruct (segstore s0 h (List.map (fun x => (x, match typeof v0 with T_handle => Handle | _ => Numeric end)) (bits v0)) (t_length (typeof v0))) eqn:HStoreResult.
 ...
 - (* Segfree *)
   ...
   destruct (List.nth_error (s_segs s) n) eqn:HN => //.
   destruct (List.nth_error (s_alls s) n0) eqn:HN0 => //.
   destruct (find_and_remove h.(id) a.(allocated)) eqn:Halloc.
   ...
   repeat eexists. eapply rm_segfree_failure; eauto.
   left. intro Habs; unfold find_address in Habs ; rewrite Halloc in Habs => //. 
...
Lemma t_progress_e: forall s C C' f vcs es tf ts1 ts2 lab ret,
    ...
    exists me s' f' es', reduce s f (v_to_e_list vcs ++ es) me s' f' es'.
Proof.
  (* e_typing *)
  ...
  - (* AI_basic *)
    ...
    destruct HType as [HType | [me [s' [vs' [es' HType]]]]].
    + left.
      unfold terminal_form; left.
  ...
  - (* Composition *)
    ...
    + (* reduce *)
      ...
      right.
      exists me, s', f', (es' ++ [::e]).
      eapply rm_label; eauto; try apply/lfilledP.
...
(*      by []. *)
    ...
    + (* reduce *)
      destruct H as [me [s' [f0' [es' HReduce]]]].
      exists me, s', f, [::AI_local (length ts2) f0' es'].
      by apply rm_local; apply HReduce.
  - (* Invoke *)
    ...
    + (* Native *)
      right.
      exists ME_empty, s, f, [:: AI_local (length ts2) (Build_frame (vcs ++ n_zeros ts) i) [::AI_basic (BI_block (Tf [::] ts2) es)]].
      eapply rm_silent, r_invoke_native; eauto.
      repeat rewrite length_is_size. by rewrite size_map.
    + (* Host *)
      right.
      exists ME_empty, s, f, [:: AI_call_host (Tf [seq typeof i | i <- vcs] ts2) h vcs].
      eapply rm_silent, r_invoke_host; eauto.
      repeat rewrite length_is_size. by rewrite size_map.
  - (* AI_label *)
...
(*    { rewrite upd_label_overwrite. simpl. eauto. } *)
    ...
    + (* reduce *)
      ...
      right.
      simpl in HReduce.
      exists me, s', f', [::AI_label (length ts) e0s es'].
      by eapply rm_label; eauto; apply label_lfilled1.
...
Theorem t_progress: forall s f es ts,
    ...
    terminal_form es \/
      (exists tf h vcs n, first_instr es = Some (AI_call_host tf h vcs,n)) \/
                        exists me s' f' es', reduce s f es me s' f' es'.
Proof.
