                  ...
                  if List.nth_error s.(s_segs) j is Some seg_s_j then
                    if List.nth_error s.(s_alls) j' is Some all_s_j' then
                      ...
                      then expect
                             ...
                             (fun seg' =>
                                ((upd_s_seg s (update_list_at s.(s_segs) j seg'), f, RS_normal (vs_to_es ves'))))
                             ((s, f, RS_normal (vs_to_es ves' ++ [::AI_trap])))
                      else ((s, f, RS_normal (vs_to_es ves' ++ [::AI_trap])))
                    else (s, f, crash_error)
                  else (s, f, crash_error))
  ...
  | AI_basic BI_slice =>
    ...
    | AI_basic BI_handleadd =>
      if ves is VAL_handle h :: VAL_int32 c :: ves' then
        ((s, f, RS_normal (vs_to_es (VAL_handle {| base := h.(base);
                  ...
                  if List.nth_error s.(s_segs) j is Some s_seg_s_j then
                    if List.nth_error s.(s_alls) j' is Some s_all_s_j' then
                      ...
                      if seg' is Some seg'' then
                        (upd_s_seg (upd_s_all s (update_list_at s.(s_alls) j' ({| allocated := (fresh_nid s_all_s_j', l, Wasm_int.N_of_uint i32m c) :: allocated s_all_s_j' |}))) (update_list_at s.(s_segs) j seg''), f,
                    ...
                    if List.nth_error s.(s_alls) j' is Some s_all_s_j' then
                      if h.(valid) && (h.(offset) == N.zero)%N then
                        expect
                          ...
                          (fun '(l, a, n) =>
                             if (a == h.(base))%N then
                               (upd_s_all s (update_list_at s.(s_alls) j' {| allocated := l |}), f, RS_normal (vs_to_es ves'))
                             else (s, f, crash_error))
                          ((s, f, crash_error))
                      else (s, f, crash_error)
                    else (s, f, crash_error)
