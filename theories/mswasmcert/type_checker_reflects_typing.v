...
From MSWasm Require Import common operations typing type_checker properties.
...
Proof with auto_rewrite_cond.
  ...
  - destruct e => //=; (try destruct f as [tn' tm']); auto_rewrite_cond; move => ? Hs Hct Hct2; simplify_type_update => //...
    ...
    + replace ([::CTA_some T_handle; CTA_some v]) with (to_ct_list [::T_handle; v]) in Hct2 => //.
      apply consume_type_agree in Hct2.
    ...
    + replace [:: CTA_some T_handle; CTA_some T_i32 ; CTA_some T_i32] with
      ...
      apply type_update_type_agree in Hct2 as (tn' & Hct & bet); subst.
      exists (tn' ++ [::T_handle; T_i32; T_i32]); split => //.
      apply bet_weakening.
      apply bet_slice => //.
    + replace [::CTA_some T_i32] with (to_ct_list [::T_i32]) in Hct2 => //.
      apply type_update_type_agree in Hct2 as (tn' & Hct & bet) ; subst.
    ...
    + replace [::CTA_some T_i32; CTA_some T_handle] with (to_ct_list [::T_i32; T_handle]) in Hct2 => //.
      ...
      apply bet_weakening, bet_handleadd => //.
    + replace [::CTA_some T_handle] with (to_ct_list [::T_handle]) in Hct2 => //.
      apply consume_type_agree in Hct2. 
    ...
    + replace ([::CTA_some v0]) with (to_ct_list [::v0]) in Hct2 => //=.
      ...
      exists (tn' ++ [::v0]); split => //.
      apply bet_weakening.
      apply bet_reinterpret => //; try by [ move/eqP in H | rewrite H1; apply/eqP].
      by intros ->. by intros ->.
Qed.
