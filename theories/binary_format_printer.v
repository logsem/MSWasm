...
Definition binary_of_handle h :=
  List.map byte_of_compcert_byte (serialise_handle h).
...
Fixpoint binary_of_be (be : basic_instruction) : list byte :=
  ...
  | BI_segload T_i64 => xc2 :: x00 :: nil
  | BI_segload T_f32 => xc3 :: x00 :: nil
  | BI_segload T_f64 => xc4 :: x00 :: nil
  | BI_segload T_handle => xc5 :: x00 :: nil
  ...
  | BI_store T_handle (Some _) _ _ => dummy
  | BI_store T_handle None a o => xc6 :: binary_of_memarg a o
  | BI_segstore T_i32 => xc7 :: x00 :: nil
  | BI_segstore T_i64 => xc8 :: x00 :: nil
  | BI_segstore T_f32 => xc9 :: x00 :: nil
  | BI_segstore T_f64 => xca :: x00 :: nil
  | BI_segstore T_handle => xcb :: x00 :: nil
  ...
  | BI_immediate (NVAL_int32 x) => x41 :: binary_of_i32 x
  | BI_immediate (NVAL_int64 x) => x42 :: binary_of_i64 x
  | BI_immediate (NVAL_float32 x) => x43 :: binary_of_f32 x
  | BI_immediate (NVAL_float64 x) => x44 :: binary_of_f64 x
(*  | BI_immediate (NVAL_handle h) => xd0 :: binary_of_handle h *)
  ...
  | BI_relop T_f32 (Relop_h _) => dummy
  | BI_relop T_f64 (Relop_h _) => dummy
  | BI_relop T_handle (Relop_h ROH_eq) => xda :: nil
  | BI_relop T_handle (Relop_h ROH_ne) => xdb :: nil
  | BI_relop T_handle (Relop_i _) => dummy
  | BI_relop T_handle (Relop_f _) => dummy
  ...
  | BI_relop T_i32 (Relop_h _) => dummy
  | BI_relop T_i64 (Relop_h _) => dummy
  ...
  | BI_cvtop T_handle _ _ _ => dummy
  | BI_cvtop _ _ T_handle _ => dummy
  end.
...
Definition binary_of_tagged_byte (x : byte * btag) :=
  let '(b, t) := x in
  match t with
  | Numeric => [:: xd7 ; b]
  | Handle => [:: xd8 ; b]
